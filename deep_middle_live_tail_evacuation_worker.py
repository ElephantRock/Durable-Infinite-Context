...
def main() -> None:
    ...
    store = DeepMiddleLiveTailEvacuationRetirementDescriptorPrimaryStore(args.file)

    if args.command == "evacuate":
        store.evacuate_deep_middle_live_retirement_arena_tail_step(
            failpoint=killer(args.failpoint)
        )
        raise AssertionError("requested v0.43 evacuation failpoint was not reached")

    if args.command == "insert":
        store.insert(args.key, failpoint=killer(args.failpoint))
        raise AssertionError("requested v0.43 insert failpoint was not reached")
