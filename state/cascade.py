...
class CascadeMaterialization:
    """Layered rebuildable state with explicit dependency lifecycle.
    ...
    The dependency graph owns only lifecycle and lineage; it does not become a
    second source of semantic truth. Derived nodes whose canonical basis disappears
    are retired after their local rebuild, preventing deleted state from surviving
    as unbounded dependency metadata.
    """
    ...
    def _state_dependencies(
        self,
        key: StateKey,
        trace: DependencyTrace | None = None,
    ) -> set[str]:
    ...
    def _profile_dependencies(
        ...
        if trace is not None:
            trace.evidence_reads += sum(
                1 for eid in evidence_ids if eid in self.store.evidence
            )
    ...
    def _retire_rebuilt_absent(
        ...
        for node_id in rebuilt_node_ids:
            ...
            if kind == "profile":
                subject_id = self._subject_by_profile_node.get(node_id)
                if subject_id is not None:
                    subjects.add(subject_id)
            elif kind in {"state", "support", "context"}:
                key = self._key_by_node.get(node_id)
                if key is not None:
                    keys.add(key)
        ...
        for subject_id in sorted(subjects):
            ...
            if (
                not assertions
                and self.graph.status_of(node_id) == DerivationStatus.FRESH
            ):
                trace.absorb(self.graph.remove_derived(node_id))
                self._subject_by_profile_node.pop(node_id, None)
        ...
        for key in sorted(keys):
            ...
            statuses = [
                ...
                if self.graph.status_of(node_id) is not None
            ]
            if any(status != DerivationStatus.FRESH for status in statuses):
                continue
    ...
    def rebuild(self, target_node_ids: Iterable[str] | None = None) -> DependencyTrace:
        ...
        self._retire_rebuilt_absent(rebuilt, trace)
        return trace
    ...
    def read_context(self, key: StateKey) -> str | None:
        ...
        if status is not None and status != DerivationStatus.FRESH:
            raise RuntimeError(
                f"Context materialization is not fresh: {node_id} ({status.value})"
            )
    ...
    def equivalent_to(self, other: "CascadeMaterialization") -> bool:
        return (
            ...
            and self.graph.equivalent_to(other.graph)
        )
